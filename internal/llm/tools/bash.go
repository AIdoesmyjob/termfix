...
var safeReadOnlyCommands = []string{
	...
	// File inspection
	"cat", "head", "tail", "less", "wc", "sort", "grep", "find", "file", "stat",

	// System services and logs
	"systemctl status", "systemctl is-active", "systemctl list-units", "systemctl --failed",
	"journalctl", "dmesg",

	// Network
	"ip addr", "ip route", "ip link", "ss", "netstat", "ping", "traceroute", "dig", "nslookup", "host",
...
func bashDescription() string {
	return `Run a bash command. Output is returned. Commands share a persistent shell session. Use ';' or '&&' to chain commands.`
}
